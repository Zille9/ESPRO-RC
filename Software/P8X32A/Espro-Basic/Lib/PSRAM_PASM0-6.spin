...
_XINFREQ     = 5_000_000
DB_IN           = %00000000_00000000_00000000_00010000  'maske: dbus-eingabe
...
pub main|x
    start
    'ram_sqi
    wr_value(196,241,Job_Poke)
    x:=rd_value(196,Job_Peek)
    repeat
...
sub_poke                mov     _tmp,_val               'kopie von _val
                        ...
                        shr     _tmp,#4                 '4bit nach rechts in Position 3..0 schieben
                        mov     outa,_tmp
                        call    #CLOCK
                        and     _val,#$F
                        mov     outa,_val
...
sub_peek
                        ...
                        mov     _tmp2,ina
                        call    #CLOCK
                        and     _tmp,ANDMASK            '4bit MSB maskieren
                        shl     _tmp,#4
                        and     _tmp2,ANDMASK           '4bit LSB maskieren
                        add     _tmp,_tmp2
...
setadr_loop
                        ...
                        shr     _tmp2,_SHIFT            '20Bits nach rechts schieben 23..20->11..8
                        and     _tmp2,ANDMASK           'SIO3..SIO0 ausmaskieren
                        mov     outa,_tmp2              'Bit 23-20 ausgeben
                        call    #CLOCK
                        sub     _SHIFT,#4
                        djnz    _CNT, #setadr_loop      'nächste runde bis _REGC 0
{
                        mov     _tmp2,_adr              '19-16
                        shr     _tmp2,#16               '8Bits nach rechts schieben 19..16 ->11..8
                        and     _tmp2,ANDMASK           'SIO3..SIO0 ausmaskieren
                        mov     outa,_tmp2              'Bit 19-16 ausgeben
                        call    #CLOCK

                        mov     _tmp2,_adr              '15-12
                        shr     _tmp2,#12               '4Bits nach rechts chieben 15..12 ->11..8
                        and     _tmp2,ANDMASK           'SIO3..SIO0 ausmaskieren
                        mov     outa,_tmp2              'Bit 15-12 ausgeben
                        call    #CLOCK

                        mov     _tmp2,_adr              '11-8
                        shr     _tmp2,#8                '4Bits nach rechts chieben 15..12 ->11..8
                        and     _tmp2,ANDMASK           'SIO3..SIO0 ausmaskieren 11..8 = 11..8
                        mov     outa,_tmp2              'Bit 11-8 ausgeben
                        call    #CLOCK

                        mov     _tmp2,_adr              '7-4
                        shr     _tmp2,#4                '4bits nach links schieben 7..4 ->11..8
                        and     _tmp2,ANDMASK           'SIO3..SIO0 ausmaskieren
...
SPIQUAD_WR              mov     outa,_SIO0_Out0           'SIO0=0 CS=0
                        mov     dira,_SIO0                'SPI zu Quad-WR Befehl $38 %0011_1000
                        call    #CLOCK
                        call    #CLOCK
...
SPIQUAD_RD              mov     outa,_SIO0_Out0           'SIO0=0 CS=0
                        mov     dira,_SIO0                'SPI zu Quad-RD Befehl $38 %1110_1011
                        mov     outa,_SIO0_Out1           'SIO0=1
                        call    #CLOCK
                        call    #CLOCK
                        call    #CLOCK
...
SPI2SQI                 mov     outa,_SIO0_Out0           'SIO0=0 CS=0
                        mov     dira,_SIO0                'SPI zu SQI Befehl $EB %1110_1011
                        call    #CLOCK
                        call    #CLOCK
                        mov     outa,_SIO0_Out1           'SIO0=1
                        call    #CLOCK
...
_DIR_OUT      long %00000000_00000000_00000000_00111111
_DIR_IN       long %00000000_00000000_00000000_00110000
_BUS_INIT     long %00000000_00000000_00000000_00000000

_SIO0         long %00000000_00000000_00000000_00110001
_SIO0_Out0    long %00000000_00000000_00000000_00000000
_SIO0_Out1    long %00000000_00000000_00000000_00000001



_COM38_A      long %00000000_00000000_00000000_00000011 'Kommando schreiben MSB $38
_COM38_B      long %00000000_00000000_00000000_00001000 'Kommando schreiben LSB

_COMEB        long %00000000_00000000_00000000_00001110 'Kommando lesen MSB $EB
_COM0B        long %00000000_00000000_00000000_00001011 'Kommando lesen LSB

CLKPIN        long |< 5

ANDMASK       long %00000000_00000000_00000000_00001111
DESELECT      long %00000000_00000000_00000000_00010000
